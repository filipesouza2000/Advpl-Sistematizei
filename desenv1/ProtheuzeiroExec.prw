#INCLUDE 'PROTHEUS.CH'
#INCLUDE 'RWMAKE.CH'
#INCLUDE 'FONT.CH' //Include responsùvel pelas fontes coloridas utilizadas no Texto, e com diversos estilos
#INCLUDE 'COLORS.CH' //Responsùvel pelas cores
...
oSay2      := TSay():New( 100,008,{||"Caso tenha parùmetros, informe entre parùnteses"},oDlg1,,oFont2,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,164,008)
...
oBtn1      := TButton():New( 076,008,"Executar",oDlg1,{|| IIF(!Empty(cNomeFun),fMontaFun(),Alert("Operaùùo nùo permitida"))},060,016,,oFont1,,.T.,,"",,,,.F. )
...
IF !("(" $ cNomeFun) .OR. !(")" $ cNomeFun)
    Alert("Vocù precisa colocar os parùnteses!","Tente novamente")
else
